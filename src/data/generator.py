import numpy as np
from src.data import preproc as pp, reader

class DataGenerator():
    """Generator class with data streaming"""

    def __init__(self, train_file_name, test_file_name, batch_size, charset, max_text_length=128, predict=False,
                 max_sentances=None, items_per_sentance=None):
        self.tokenizer = Tokenizer(charset, max_text_length)
        self.batch_size = batch_size
        self.partitions = ['test'] if predict else ['train', 'test']

        self.size = dict()
        self.steps = dict()
        self.index = dict()

        dt_train = reader.read_from_txt(train_file_name, max_sentances, items_per_sentance)
        dt_test = reader.read_from_txt(test_file_name, max_sentances, items_per_sentance)
        self.dataset = {"train": dt_train, "test": dt_test}
    ...
    def next_train_batch(self):
        ...
        while True:
            if self.index['train'] >= self.size['train']:
                ...
                self.dataset['train']['dt'] = [self.dataset['train']['dt'][i] for i in self.arange]
                self.dataset['train']['gt'] = [self.dataset['train']['gt'][i] for i in self.arange]
            ...
            # targets = np.array(self.dataset['train']['gt'])[index:until]
            # inputs = targets if self.noise_process else np.array(self.dataset['train']['dt'])[index:until]

            targets = [self.dataset['train']['gt'][i] for i in range(index, until) if i < len(self.dataset['train']['gt'])]
            inputs = targets if self.noise_process else [self.dataset['train']['dt'][i] for i in range(index, until) if i < len(self.dataset['train']['gt'])]
